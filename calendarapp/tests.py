...
class EventListViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.all_tasks_url = reverse('calendarapp:all_tasks')  # URL for AllTasksListView
        self.today_tasks_url = reverse('calendarapp:today_tasks')  # URL for TodayTasksListView
        self.upcoming_tasks_url = reverse('calendarapp:upcoming_tasks')  # URL for UpcomingTasksListView
        self.incompleted_tasks_url = reverse('calendarapp:incompleted_tasks')  # URL for IncompletedTasksListView

        # Creating tasks for testing
        self.task1 = Task.objects.create(title="Past Task", user=self.user, due_date="2023-10-01")  # Completed task
        self.task2 = Task.objects.create(title="Today Task", user=self.user, due_date="2024-11-01")  # Running task
        self.task3 = Task.objects.create(title="Upcoming Task", user=self.user, due_date="2024-12-01")  # Upcoming task
        self.task4 = Task.objects.create(title="Incompleted Task", user=self.user, due_date="2023-09-01")  # Completed task
    ...
    def test_all_tasks_view(self):
        """ Test that all tasks are listed correctly """
        response = self.client.get(self.all_tasks_url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'calendarapp/task_list.html')
        ...
        self.assertIn(self.task4, response.context['object_list'])

    def test_today_tasks_view(self):
        """ Test that only today tasks are listed """
        response = self.client.get(self.today_tasks_url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'calendarapp/tasks_list.html')
        self.assertNotIn(self.task2, response.context['object_list'])  # Today task should be in the list
    
    def test_upcoming_tasks_view(self):
        """ Test that only upcoming tasks are listed """
        response = self.client.get(self.upcoming_tasks_url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'calendarapp/tasks_list.html')
        self.assertIn(self.task3, response.context['object_list'])
        self.assertNotIn(self.task1, response.context['object_list'])
        self.assertNotIn(self.task2, response.context['object_list'])
        self.assertNotIn(self.task4, response.context['object_list'])
    
    def test_incompleted_tasks_view(self):
        """ Test that only incompleted tasks are listed """
        response = self.client.get(self.incompleted_tasks_url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'calendarapp/tasks_list.html')
        self.assertIn(self.task4, response.context['object_list'])
        self.assertNotIn(self.task1, response.context['object_list'])
